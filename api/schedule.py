...
BASE_SCHEDULE_URL = os.environ.get("BASE_SCHEDULE_URL", config.BASE_SCHEDULE_URL_CONFIG)
...
def api_schedule():
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(
            scrape_schedule, username, password, BASE_SCHEDULE_URL, 3, 2
        )
        try:
            data = future.result(timeout=10)
        except TimeoutError:
            log_event(f"Timeout during schedule scraping for user: {username}")
            return jsonify({"error": "Scraping timed out"}), 504
        except Exception as e:
            log_event(f"Error during schedule scraping for user: {username} - {str(e)}")
    ...
    if data:
        ...
        log_event(f"Failed to scrape schedule for user: {username}")
        return jsonify({"error": "Failed to fetch schedule data"}), 500
